...
//primero resuelvo la vista para que pinte una pantalla con botones
    ...
    public int botonPulsado(MouseEvent e)
    {
        ...
        if(gameOver ==false){
            ...
            if(e.getButton() == MouseEvent.BUTTON1)
            {
                ...
                if(b.mina == 1){
                    ...
                    nombre = JOptionPane.showInputDialog("Escriba su usuario");
                    String pp = JOptionPane.showInputDialog("Escriba su edad");
        
                    int edad = Integer.parseInt(pp);
                    
                    String po = JOptionPane.showInputDialog("Si desea guardar su nombre con su puntaje presione 1");
        
                    int sino = Integer.parseInt(po);
                       
                    gameOver = true;
                    
                        ArrayList Jugador = new ArrayList();
                       
                        //se juntan los datos en un solo String porque se escriben cosas
                        //muy raras si no se juntan, como por ejemplo: Ԁt‧†丠浯牢㩥瀠灥⁥†摅摡›㈱†倠湵潴㩳㐠
                        String datos = "    Nombre: "+nombre+"   Edad: "+edad+"   Puntos: "+puntos+"    ";
                        Jugador.add(datos);

                    if(sino == 1){                
                    
                        try{
                        //Guarda los datos solamente del ultimo jugador
                        ObjectOutputStream escribir = new ObjectOutputStream(new FileOutputStream("Jugador.txt") );
                        escribir.writeObject(Jugador);
                        escribir.close();
                        }catch (Exception u) {
                        JOptionPane.showMessageDialog(null,""+u.getMessage());
                        }
                    }      
                    String pe = JOptionPane.showInputDialog("Si desea ver otros puntajes presione 1");
                    int nonon = Integer.parseInt(pe);   
                    if(nonon == 1){
        ...
        //Este return no entiendo para que sea pero netbeans me obligó a ponerlo
        return puntos;
    }
